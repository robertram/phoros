...
const FooterMailchimpNewsletter = () => {
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="items-start mx-auto mb-3 max-w-screen-sm sm:flex sm:space-y-0 flex flex-col md:flex-row m-auto ">
        ...
        <div className='md:ml-[20px] md:mt-0 mt-[20px] '>
          {state != 2 &&
            <button type="submit" className={`whitespace-nowrap py-3 px-5 w-full text-sm font-medium text-center text-white rounded-lg cursor-pointer sm:rounded-none sm:rounded-r-lg hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 hover:scale-110 transition ease-in-out delay-1000 ${gradient}`}>{loading ? 'Loading' : 'Join the waitlist'}</button>
          }
          {state == 2 && <button disabled type="button" className="py-3 px-5 w-full text-sm font-medium text-center text-white rounded-lg border cursor-auto bg-primary-700 border-primary-600 sm:rounded-none sm:rounded-r-lg hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800 whitespace-nowrap">Thanks for subscribing!</button>}
        </div>
...
export default FooterMailchimpNewsletter
